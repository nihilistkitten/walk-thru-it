...
class Point2d {
  ...
  minus(other) {
    if (other instanceof Point2d) {
      ...
      //
      return new Point2d(this.x - other.dx, this.y - other.dy);
    } else {
      return this;
    }
  }
...
// Compute the scalar which identifies the intersection point along P, without verifying this point exists.
function intersection_unchecked(p0, p1, q0, q1) {
  // compute Q basis
  var uhat = q1.minus(q0).unit();
  var vhat = uhat.perp();

  // compute perpendicular component of P
  var p0v = p0.minus(q0).dot(vhat);
  var p1v = p1.minus(q0).dot(vhat);

  // identify percentage along P where the intersection lies
  // because P is linear, we only care about the perpendicular component
  return p0v / (p0v - p1v);
}

// Compute the scalar which identifies the intersection point along the segment P, or null if no intersection exists.
function intersection(p0, p1, q0, q1) {
  var alpha = intersection_unchecked(p0, p1, q0, q1);
  ...
  var beta = intersection_unchecked(q0, q1, p0, p1);
