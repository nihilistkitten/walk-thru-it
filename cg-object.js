...
class Edge {
  ...
  /// Return the affine combination of the vertices at the given alpha.
  combo(alpha, owner) {
    start = this.vertex(0, owner);
    end = this.vertex(1, owner);
    return start.combo(alpha, end);
  }
...
class CGObject {
  ...
  regirthAndSeat() {
    ...
    for (let V of this.allVertices()) {
      const r2 = V.position.x * V.position.x + V.position.y * V.position.y;
      if (r2 > radius2) {
        radius2 = r2;
      }
  ...
  //
  recenter() {
    let maxDims = new Point3d(
      Number.MIN_VALUE,
      Number.MIN_VALUE,
      Number.MIN_VALUE
    );
    let minDims = new Point3d(
      Number.MAX_VALUE,
      Number.MAX_VALUE,
      Number.MAX_VALUE
    );
